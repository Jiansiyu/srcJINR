...
void BmnMwpcTrackFinder::Exec(Option_t* opt, TClonesArray* fBmnMwpcSegmentsArray, TClonesArray* fBmnMwpcTracksArray) {
	if (!IsActive()) return;
	...
	for (Int_t iChamber = 0; iChamber < kNChambers; iChamber++) {
		...
		if ( Nbest_Ch[iChamber] > 0){
			...
			for (Int_t ise = 0; ise < Nbest_Ch[iChamber]; ise++) {
				...
				if (fDebug) cout<<" iChamber "<<iChamber<<" ax= "<<par_ab_Ch[iChamber][0][ise]<<" bx= "<<par_ab_Ch[iChamber][1][ise]<<" ay= "<<par_ab_Ch[iChamber][2][ise]<<" by= "<<par_ab_Ch[iChamber][3][ise]<<endl;
			}
		}
	}

	if (fDebug)  cout<<" N0= "<<Nbest_Ch[0]<<" N1= "<<Nbest_Ch[1]<<" N2= "<<Nbest_Ch[2]<<" N3= "<<Nbest_Ch[3]<<endl;
	...
	if ( Nbest_Ch[2] > 0 && Nbest_Ch[3] > 0)  
		SegmentMatching( 2, Nbest_Ch, par_ab_Ch, kZmid, ind_best_Ch, Nbest_pair,  Chi2_match_pair, XVU_Ch, Nhits_Ch, Nhits_match);
	...
	for (Int_t p = 0; p < kNumPairs; p++) {
		for (Int_t se = 0; se < 5; se++) {
			if (Chi2_match_pair[p][se] != 999.) {
				...
				//if (fDebug)   cout<<" pair "<<p<<" se "<<se<<" Chi2_match "<<Chi2_match_pair[p][se]<<endl;
			}
		}
	}
	...
	Double_t theta, phi; Float_t X_par_to_target, Y_par_to_target;
	for (Int_t iPair = 0; iPair < kNumPairs; iPair++) {
		if ( Nbest_pair[iPair] > 0){
			...
			/*
			   if ( Nbest_pair[0] > 0 && Nbest_Ch[3] > 0 ) 
			   StraightByTwoPoints(par_ab_pair, Nbest_pair, par_ab_Ch, Nbest_Ch, kZ_midle_pair, par_ab_3ch, Nbest_3ch);
			   for (Int_t itr = 0; itr < Nbest_3ch[0]; itr++) {

			   BmnTrack *Tr_Todc = new ((*fBmnMwpcTracksArrayToDC)[fBmnMwpcTracksArrayToDC->GetEntriesFast()]) BmnTrack();

			   hpar_Ax_3ch -> Fill( par_ab_3ch[0][itr]);
			   hpar_Bx_3ch -> Fill( par_ab_3ch[1][itr]);
			   hpar_Ay_3ch -> Fill( par_ab_3ch[2][itr]);
			   hpar_By_3ch -> Fill( par_ab_3ch[3][itr]);

			   Tr_Todc -> SetFlag(itr);
			   FairTrackParam TrParamsTodc;
			   TrParamsTodc.SetPosition(TVector3( par_ab_3ch[1][itr],  par_ab_3ch[3][itr],kZ_DC));
			   TrParamsTodc.SetTx( par_ab_3ch[0][itr]);
			   TrParamsTodc.SetTy( par_ab_3ch[2][itr]);
			   Tr_Todc -> SetParamFirst(TrParamsTodc);
			   }
			   */
		}//> 0
	}//iPair
...
void BmnMwpcTrackFinder::SegmentParamAlignment(Int_t chNum, Int_t *Nbest,  Double_t ***par_ab, Float_t **shiftt ){
	...
	for (Int_t iBest = 0; iBest < Nbest[chNum]; iBest++) {
		...
		// cout<<" chNum "<<chNum<<" after Alignment: iBest "<<iBest<<" Ax "<< par_ab[chNum][0][iBest]<<" bx "<< par_ab[chNum][1][iBest]<<" Ay "<< par_ab[chNum][1][iBest]<<" by "<< par_ab[chNum][3][iBest]<<endl;

	}//iBest 
}//SegmentParamAlignment


void BmnMwpcTrackFinder::StraightByTwoPoints(Double_t ***par_ab_p, Int_t *Nbest_p, Double_t ***par_ab_ch, Int_t *Nbest_ch, Float_t *Z_midle_pair, Double_t **par_ab_3, Int_t *Nbest_3){
	...
	//if (fDebug) cout<<" Nbest_3 "<<Nbest_3[0]<<endl;
	Int_t Nbest_ = 0;

	for (Int_t pair1 = 0; pair1 < Nbest_p[0]; pair1++) {
		if ( pair1 ==  best1) continue;
		//pair 0                                 zloc0 -z_i
		Float_t x1 = par_ab_p[0][0][pair1] *( ZCh[3] - kZ_midle_pair[0] ) + par_ab_p[0][1][pair1] ;
		Float_t y1 = par_ab_p[0][2][pair1] *( ZCh[3] - kZ_midle_pair[0] ) + par_ab_p[0][3][pair1] ;

		// if (fDebug)cout<<"pair1 "<<pair1<<" y1 "<<y1<<" Ay "<<par_ab_p[0][2][pair1]<<" by "<<par_ab_p[0][3][pair1]<<endl;

		// hXpair0_inCh3->Fill(x1);
		//  hYpair0_inCh3->Fill(y1);

		for (Int_t bst2 = 0; bst2 < Nbest_ch[3]; bst2++) {
			if ( bst2 ==  best2) continue;
			//      cout<<" bst2 "<<bst2<<"  best2 "<<best2<<endl;

			//par_pair - par_ch3
			min_distX = x1 - par_ab_ch[3][1][bst2]; //min
			min_distY = y1 - par_ab_ch[3][3][bst2]; //min

			//  hdX_inCh3->Fill(min_distX);
			// hdY_inCh3->Fill(min_distY);
			...
			if (Chi2_m < min_Chi2m ){
				...
				best1 = pair1;
				best2 = bst2;
				par_ab_3[0][Nbest_] = (par_ab_ch[3][1][bst2] - par_ab_p[0][1][pair1])/(ZCh[3] - kZ_midle_pair[0]);
				par_ab_3[2][Nbest_] = (par_ab_ch[3][3][bst2] - par_ab_p[0][3][pair1])/(ZCh[3] - kZ_midle_pair[0]);
				// x_extr            =                  Ax       (dz)          + bx // extrap to glob dc
				par_ab_3[1][Nbest_] = par_ab_3[0][Nbest_] * (kZ_DC - ZCh[3] ) + par_ab_ch[3][1][bst2];
				par_ab_3[3][Nbest_] = par_ab_3[2][Nbest_] * (kZ_DC - ZCh[3] ) + par_ab_ch[3][3][bst2];
...
void BmnMwpcTrackFinder::SegmentMatching( Int_t first_Ch, Int_t *Nbest, Double_t ***par_ab, Float_t *Zmid, Int_t **best_Ch, Int_t *Nbest_pair_, Double_t **Chi2_match_, Double_t ***XVU_Ch_, Int_t **Nhits_, Int_t **Nhits_m){
	...
	Float_t sig_dx= 3.6; 
	Float_t sig_dy= 3.6; 
	Float_t sig_dax= 0.055; 
	Float_t sig_day= 0.055; 

	Float_t min_Chi2m = 50;
	Float_t min_distX = 99;
	Float_t min_distY = 99;
	Float_t dAx12 = 0;
	Float_t dAy12 = 0;
	...
	if (Nbest_Ch[first_Ch] > 0 && Nbest_Ch[Secon_Ch] > 0){ 
		...
		for (Int_t bst1 = 0; bst1 < Nbest[first_Ch]; bst1++) {	  
			...
			for (Int_t bst2 = 0; bst2 < Nbest[Secon_Ch]; bst2++) {
				...
				//	cout<<" Pairr "<<Pairr<<" sigma ax "<<sigma_delta[Pairr][0]<<" ay "<<sigma_delta[Pairr][2]<<" sigma x "<<sigma_delta[Pairr][1]<<" y "<<sigma_delta[Pairr][3]<<endl;
				...
				//	cout<<" Pairr "<<Pairr<<" bst1 "<<bst1<<" Nhits "<<Nhits_[first_Ch][bst1]<<" bst2 "<<bst2<<" Nhits "<<Nhits_[Secon_Ch][bst2]<<" Chi2_m "<<Chi2_m<<endl;

				if (Chi2_m < min_Chi2m && Nvariat < Nvariations ){ 
...
void BmnMwpcTrackFinder::PairMatching( Int_t *Nbest_p, Double_t ***par_ab_p,  Float_t *kZ_midle_pair_){
	...
	for (Int_t pair0 = 0; pair0 < Nbest_p[0]; pair0++) {	  
		...
		// cout<<" pair "<<0<<" pair0 "<<pair0<<" ax "<<par_ab_p[0][0][pair0]<<" bx "<< par_ab_p[0][1][pair0]<<" ay "<<par_ab_p[0][2][pair0]<<" by "<<par_ab_p[0][3][pair0]<<" kZ_target "<<kZ_target<<" kZ_midle_pair "<<kZ_midle_pair_[0]<<endl;
		...
		for (Int_t pair1 = 0; pair1 < Nbest_p[1]; pair1++) {
			...
			// cout<<" pair "<<1<<" pair1 "<<pair1<<" ax "<<par_ab_p[1][0][pair1]<<" bx "<< par_ab_p[1][1][pair1]<<" ay "<<par_ab_p[1][2][pair1]<<" by "<<par_ab_p[1][3][pair1]<<" kZ_target "<<kZ_target<<" kZ_midle_pair "<<kZ_midle_pair_[1]<<endl;
			...
			if (Chi2_m < min_Chi2m){
				...
				htheta_p1vsp0 -> Fill(TMath::RadToDeg()*theta0,TMath::RadToDeg()*theta1);

			}

			//  cout<<" Npairs "<<Npair_dist<<endl;
			// cout<<" min_Chi2m "<<min_Chi2m<<endl;

		}//pair1
	}//pair0

	hChi2_m_target->Fill(min_Chi2m);
...
void BmnMwpcTrackFinder::SegmentFit(Int_t First_Ch, Float_t **z_gl_, Float_t *sig,Int_t *Nbest_pair_, Int_t ** ind_best_Ch_, Double_t ***par_ab_pair_,  Double_t **Chi2_ndf_pair_, Double_t ***XVU_Ch_, Double_t ***Clust_Ch_, Int_t **ind_best_pair_,Int_t **Nhits_Ch_, Int_t **Nhits_pair_){
	...
	for (Int_t bst = 0; bst < Nbest_pair_[Pair1]; bst++) { 
		...
		for(Int_t i = 0; i < 6; i++){
			sigm_1[i]= 0.; sigm_2[i]= 0.; 
			h1[i] = 0; h2[i] = 0;
			if ( XVU_Ch_[fir][i][best1]  > -999.) {
				h1[i] = 1;
				sigm_1[i] = (Clust_Ch_[fir][i][best1]*dw)/sq12;
				sigm2_1[i] = sigm_1[i]*sigm_1[i];
			}//if coord was

			if ( XVU_Ch_[sec][i][best2]  > -999.) {
				h2[i] = 1;
		...
		if (fDebug){
			for(Int_t i = 0; i < 6;i++){
				cout<<" h1 "<<h1[i]<<" sigm2 "<< sigm2_1[i]<<" XVU_Ch1 "<<XVU_Ch_[fir][i][best1]<<" z "<<z_gl_[fir][i]<<endl;
			}  
			cout<<endl;
			for(Int_t i = 0; i < 6;i++){
				cout<<" h2 "<<h2[i]<<" sigm2 "<< sigm2_2[i]<<" XVU_Ch2 "<<XVU_Ch_[sec][i][best2]<<" z "<<z_gl_[sec][i]<<endl;
			}
		}
...
InitStatus BmnMwpcTrackFinder::Init() {
	...
	//fBmnMwpcTracksArrayToDC = new TClonesArray("BmnMwpcTrackToDC");
	//ioman ->Register("BmnMwpcTrackToDC", "MWPC", fBmnMwpcTracksArrayToDC, kTRUE);
	...
	for (int i=0; i < kNumPairs; ++i){ 
		...
		h = new TH1D(Form("dX_Zmid_pair%d",i), Form("dX_Zmid_pair%d; cm;  Events",i), 100, -30.,30.);  fList.Add(h); hdX_Zmid_pair.push_back(h);
		h = new TH1D(Form("dY_Zmid_pair%d",i), Form("dY_Zmid_pair%d; cm;  Events",i), 100, -30.,30.);  fList.Add(h); hdY_Zmid_pair.push_back(h);
		h = new TH1D(Form("dAx_Zmid_pair%d",i),Form("dAx_Zmid_pair%d; rad;Events",i), 100, -.5,.5);    fList.Add(h); hdAx_Zmid_pair.push_back(h);
		h = new TH1D(Form("dAy_Zmid_pair%d",i),Form("dAy_Zmid_pair%d; rad;Events",i), 100, -.5,.5);    fList.Add(h); hdAy_Zmid_pair.push_back(h);
	...
	hpar_Ax_3ch  =   new TH1D("par_Ax_3ch", " tgx by 3ch-s to DC; ; Events ", 100, -0.05, 0.05);                                     fList.Add(hpar_Ax_3ch);
	hpar_Bx_3ch  =   new TH1D("par_Bx_3ch", " pos x by 3ch-s to DC; [cm]; Events ", 100, -30., 30.);                                 fList.Add(hpar_Bx_3ch);
	hpar_Ay_3ch  =   new TH1D("par_Ay_3ch", " tgy by 3ch-s to DC; ; Events ", 100, -0.05, 0.05);                                     fList.Add(hpar_Ay_3ch);
	hpar_By_3ch  =   new TH1D("par_By_3ch", " pos y by 3ch-s to DC; [cm]; Events ", 100, -30., 30.);                                 fList.Add(hpar_By_3ch);
	...
	for(Int_t i = 0; i < kNumPairs; i++){
		...
		i1=i;
		sigma_delta[0][0] =.0624;// sigm_dax                                                                                    
		sigma_delta[0][2] =.066; // sigm_day                                                                                  
		sigma_delta[0][1] = 4.08;// sigm_dx                                                           
		sigma_delta[0][3] = 4.30;// sigm_dy 

		if (i == 1) {
			i1=2;
			sigma_delta[0][0] = 0.13;  sigma_delta[1][0] =.0624;// sigm_dax  
			sigma_delta[0][2] = 0.07;  sigma_delta[1][2] =.066; // sigm_day                                                        
			sigma_delta[0][1] = 5.8;   sigma_delta[1][1] = 4.08;// sigm_dx                                                                                          
			sigma_delta[0][3] = 3.0;   sigma_delta[1][3] = 4.30;// sigm_dy 
		}
