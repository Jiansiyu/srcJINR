	...
	for( int fi = 0 ; fi < files ; ++fi){
		...
		for (int event=0 ; event<nEvents ; event++){
			...
			for( int en = 0 ; en < tofData->GetEntriesFast() ; en++ ){
				...
				if( numHits[plane][strip][side] != 0){ // i.e. this side already has a registered hit, then we want the earliest time hit
					if( tofT < times[plane][strip][side]){
				...
				if( times[plane][strip][0] != 0. && times[plane][strip][1] != 0. && \
					...
					if( numHits[plane][strip][0] == 1 && numHits[plane][strip][1] == 1 && flagHit[plane][strip] == 0){ // we have a matched hit
						...
						// Save the strip information and then clear the strip information
						...
						times[plane][strip][0] = 0.;
						times[plane][strip][1] = 0.;
						amps[plane][strip][0] = 0.;
						amps[plane][strip][1] = 0.;
			...
			for( int pl = 0 ; pl < 20 ; pl++)
				for( int st = 0 ; st < 48 ; st++){
					if( numHits[pl][st][0] == 1 ) hHits[pl]->Fill( st , 0 );
					if( numHits[pl][st][1] == 1 ) hHits[pl]->Fill( st , 1 );
					if( numHits[pl][st][0] == 2 ) hHits[pl]->Fill( st , 2 );
					if( numHits[pl][st][1] == 2 ) hHits[pl]->Fill( st , 3 );
					if( numHits[pl][st][0] > 2 ) hHits[pl]->Fill( st , 4 );
					if( numHits[pl][st][1] > 2 ) hHits[pl]->Fill( st , 5 );
				}
