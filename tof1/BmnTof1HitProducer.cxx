...
InitStatus 		BmnTof1HitProducer::Init() 
{
	...
	aTofHits = new TClonesArray("BmnTOF1Conteiner");
	...
	if (!fUseMCData) {
		...
		for (Int_t i = 0; i < 20; i++) {
			...
			pDetector[i] = new BmnTOF1Detector(i, 1, NULL);
			string path = "/input/TOF400_LRcorr_RUN7_SRC.dat";
			pDetector[i] -> SetCorrLR(path);
			pDetector[i] -> SetStripShift( "/input/TOF400_StripShift_RUN7_SRC.dat" );
			pDetector[i] -> SetWalkFunc( "/input/TOF400_TimeWalk_RUN7_SRC.dat" );
			path = "/macro/run/current_geo_file.root";
...
void 		BmnTof1HitProducer::Exec(Option_t* opt) {
	...
	else
	{
		...
		int nT0Digits = aExpDigitsT0->GetEntriesFast();
		if( nT0Digits == 1 ){
			BmnTrigDigit * digT0 = (BmnTrigDigit*) aExpDigitsT0->At(0);
			double t0Time = digT0->GetTime();
			double t0Amp = digT0->GetAmp();
			
			for( int pl = 0 ; pl < 20 ; pl++)
				pDetector[pl]->ClearHits();

			for( int en = 0 ; en < aExpDigits->GetEntriesFast(); en++){
				BmnTof1Digit * signal = (BmnTof1Digit*) aExpDigits->At(en);
				pDetector[ signal->GetPlane() ] -> InitSkim( signal );
			}
			for( int en = 0 ; en < aExpDigits->GetEntriesFast(); en++){
				BmnTof1Digit * signal = (BmnTof1Digit*) aExpDigits->At(en);
				pDetector[ signal->GetPlane() ] -> CreateStripHit( signal, t0Time, t0Amp );
			}
			for( int pl = 0 ; pl < 20 ; pl++){
				int stripMult = pDetector[pl]->GetStripMult();
				if( stripMult == 0 ) continue;
				pDetector[pl]->ClusterHits();
				pDetector[pl]->OutputToTree( aTofHits );
			}
	...
	if (fDoTest) {
		...
		//if (!fUseMCData) 
			//for (Int_t i = 0; i < fNDetectors; i++)
				//pDetector[i] -> OutputToTree(fTestFlnm.Data());
	}
