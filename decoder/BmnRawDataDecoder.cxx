...
BmnRawDataDecoder::BmnRawDataDecoder(TString file, ULong_t nEvents, ULong_t period) {
	...
	fPedoCounter = 0;
	fGemMap = NULL;
	fEvForPedestals = N_EV_FOR_PEDESTALS; // Defined in BmnAdcProcessor.h/.cxx
	fBmnSetup = kBMNSETUP;
	fT0Map = NULL;
	//InitMaps();
}
...
Int_t BmnRawDataDecoder::GetRunIdFromFile(TString name) {
	...
	fclose(file);
	if (runId <= 0) {
		printf("Couldn't read in a run number from file so parsing string\n");
		Int_t run = 0;
